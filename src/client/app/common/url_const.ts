// BACK - END, SERVIÇOS
const URL_BACK_END: string = 'http://localhost:8180/petshopCuidarRest/';

// SERVIÇO DE AUTENTICAÇÃO
export const URL_AUTH: string = URL_BACK_END + 'auth';
export const URL_REGISTRAR: string = URL_BACK_END + 'registrarUsuario';

// SERVIÇOS DE USUAŔIO
export const URL_INCLUIR_USUARIO: string = URL_BACK_END + 'incluirUsuario';
export const URL_ALTERAR_USUARIO: string = URL_BACK_END + 'alterarUsuario';
export const URL_RECUPERAR_USUARIOS_ATIVO: string = URL_BACK_END + 'recuperarUsuariosAtivo';

// SERVIÇOS DE ROLE
export const URL_RECUPERAR_ROLES: string = URL_BACK_END + 'recuperarRoles';
